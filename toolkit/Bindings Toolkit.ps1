...
param(
    ...
    [ValidateSet('NXT', 'SOL-R', 'Gunfighter', 'VMAX', 'MOZA')]
    [string]$Stick
)
...
# =====================================================================
#  STICK REGISTRY -- all supported sticks + their MFD bind tables.
#  The MFD bind table is the ONLY stick-specific data; every other
#  operation (invert / clear / restore / diagnostic / prune) is generic.
#  When a stick's MFD vJoy slots change for a patch, update its Binds.
# =====================================================================

$Registry = [ordered]@{
    'NXT' = @{
        StickName   = '[Enhanced] Dual VKB Gladiator NXT'
        LayoutMatch = 'layout_ENH_NXT_*.xml'
        Binds = @(
    ...
    'SOL-R' = @{
        ...
        LayoutMatch = 'layout_ENH_SOL-R2_*.xml'
        Binds = @(
    ...
    'Gunfighter' = @{
        ...
        LayoutMatch = 'layout_ENH_GF_*.xml'
        Binds = @(
    ...
    'VMAX' = @{
        ...
        LayoutMatch = 'layout_ENH_VMAX_AERO_*.xml'
        Binds = @(
    ...
    'MOZA' = @{
        ...
        LayoutMatch = 'layout_ENH_MTQ_MHG_*.xml'
        Binds = @(
...
# Stamped by the zip-builder when this toolkit is bundled into a specific
# stick's download. Leave empty in the source repo -- empty falls through
# to auto-detect, then a menu.
$BundledStick = ''

function Resolve-Stick {
    param([string]$Root)

    # 1. Explicit -Stick parameter wins.
    if ($Stick) { return $Stick }

    # 2. Bundled marker (set when packaged into a stick's download).
    if ($BundledStick -and $Registry.Contains($BundledStick)) { return $BundledStick }

    # 3. Auto-detect from installed layout XMLs across channels.
    $channels = @('LIVE', 'PTU', 'EPTU', 'HOTFIX', 'TECH-PREVIEW') |
        Where-Object { Test-Path -LiteralPath (Join-Path $Root $_) }
    $detected = foreach ($k in $Registry.Keys) {
        $hit = $false
        foreach ($ch in $channels) {
            $mapDir = Join-Path $Root "$ch\user\client\0\controls\mappings"
            if ((Test-Path -LiteralPath $mapDir) -and
                (Get-ChildItem -LiteralPath $mapDir -Filter $Registry[$k].LayoutMatch -ErrorAction SilentlyContinue)) {
                $hit = $true; break
            }
        }
        if ($hit) { $k }
    ...
    # 4. Menu -- pre-pointing at a single detected stick.
    $keys = @($Registry.Keys)
    Write-Host ""
    Write-Host "  Which stick are you using?" -ForegroundColor Cyan
    for ($i = 0; $i -lt $keys.Count; $i++) {
        $tag = if ($keys[$i] -in $detected) { '  (detected)' } else { '' }
        Write-Host ("    [{0}] {1}{2}" -f ($i + 1), $Registry[$keys[$i]].StickName, $tag)
    }
    $default = if ($detected.Count -eq 1) { [array]::IndexOf($keys, $detected[0]) + 1 } else { 0 }
    $promptText = if ($default) { "  Pick stick [$default]" } else { "  Pick stick" }
    $choice = (Read-Host $promptText).Trim()
    if (-not $choice -and $default) { return $keys[$default - 1] }
    if ($choice -match '^\d+$' -and [int]$choice -ge 1 -and [int]$choice -le $keys.Count) {
        return $keys[[int]$choice - 1]
    }
    throw 'No stick selected.'
}

$stickKey  = Resolve-Stick -Root $InstallRoot
$StickName = $Registry[$stickKey].StickName
$Binds     = $Registry[$stickKey].Binds
